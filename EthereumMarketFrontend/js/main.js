    ...
    for(i = 0; i < row; i++) {
      ...
      for(j = 0; j < col; j++) {
        ...
        if(idx < _numItems) {
          ...
          var states = document.createElement("div"); // 取引の状態を表示する
          var buttons = document.createElement("div"); // 取引を進めるボタンを表示する

          // IDを指定する
          image.id = "image" + idx;
          description.id = "description" + idx;
          states.id = "states" + idx;
          buttons.id = "buttons" + idx;
          ...
          label.textContent = "出品者または購入者の評価を選択してください";
  ...
  function showImage(idx) {
    contract.methods.images(idx).call().then(function(image) {
      ...
      image.addEventListener("load", function() {
        ...
        image.width = orgWidth * (image.height / orgHeight); // 高さの横幅の変化割合に合わせる
        image.style.borderRedius = "10px";
  ...
  function showDescription(idx) {
    ...
    contract.methods.items(idx).call().then(function(item) {
      for(var i = 0; i < itemIdxList.length; i++) {
        ...
        // 出品状態のみ、true⇒売り切れ、false⇒出品中、に表示を変更する
        if(i == 3) {
          if(item[itemIdxList[i]]) {
            elem.textContent = itemKeyList[i] + " : 売り切れ"
          } else {
